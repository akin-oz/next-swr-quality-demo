...
export default function ItemDetailPage({
  ...
  const { data, status, error } = useQuery(`items:${id}`, () =>
    getItem(id, 5000),
  );
  ...
  return (
    <main className="mx-auto max-w-2xl p-6 space-y-4">
      ...
      {status === "error" && (
        <Alert
          title={isNotFound ? "Not Found" : userErr?.title}
          severity={isNotFound ? "warning" : userErr?.severity}
        >
          {isNotFound
            ? `No item with id ${id}`
            : (userErr?.description ?? "Unknown error")}
        </Alert>
      )}
